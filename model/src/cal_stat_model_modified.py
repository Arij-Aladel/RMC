...
from .cal_stat_dreader_BERT_after import DNetwork_BERT
...
class DocReaderModel(object):
    ...
    def update(self, batch, vocab=None,smooth=-1, rep_train=0.5):
        ...
        if self.opt['model_type'] == 'san':
            ...
            doc_mem, query_mem, doc_mask, loss_doc, loss_query = self.network.encoder(batch, vocab)        
        else:
            raise ValueError('Unknown model type: {}'.format(self.opt['model_type']))
...
#         batch_size = len(batch["doc_tok"])#batch.size(0) #  query_mem shape: batch_size * query_hidden_size
...
#  #       print("hidden from update after bridge: ", hidden)
    ...
    def predict(self, batch,vocab=None, top_k=2):
        ...
        #TODO: ADDED
            ...
            doc_mem, query_mem, doc_mask, doc_loss, query_loss = self.network.encoder(batch, vocab)      
        else:
            raise ValueError('Unknown model type: {}'.format(self.opt['model_type']))
