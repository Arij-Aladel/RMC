...
class SANDecoder(nn.Module):
    def __init__(self, x_size, h_size, opt={}, prefix='answer', dropout=None):
        ...
        self.prefix = prefix
        # self.attn_b  = FlatSimilarityWrapper(x_size, h_size, prefix, opt, dropout) # TODO: deleted
        ...
        #self.num_turn = opt.get('{}_num_turn'.format(prefix), 5)  # TODO: deleted
        self.opt = opt
        # self.mem_random_drop = opt.get('{}_mem_drop_p'.format(prefix), 0) TODO deleted
        ...
        #self.mem_type = opt.get('{}_mem_type'.format(prefix), 0) # TODO deleted
        #self.gamma = opt.get('{}_mem_gamma'.format(prefix), 0.5) TODO deleted
        #self.alpha = Parameter(torch.zeros(1, 1, 1)) TODO deleted
    ...
    def forward(self, input, hidden, context, context_mask): # word, hidden, doc_mem, doc_mask
        ...
        if self.opt['model_type'] in{'san', 'BERT'}: # TODO: BERT ADDED in{'san', 'BERT'} or == 'san'     
            ...
            # print("new_hidden shape: ", new_hidden.shape)
            # return
