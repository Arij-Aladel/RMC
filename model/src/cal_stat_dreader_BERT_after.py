...
class DNetwork_BERT(nn.Module):
    ...
    def forward(self, batch, vocab):
        ...
        #print("BERT max len: ", max_len, "self.opt['max_doc']: ", self.opt['max_doc'] , "fix_d_len: ", fix_d_len)
        ...
        for i in range(batch_size):
            ...
            # # if len(q_text) > fix_q_len:
            # #     print("wow len(q_text): ", len(q_text))
            # q_text = " ".join(q_text)
           ...
           # print("len(input_ids_q) , fix_q_len + 2", len(input_ids_q), fix_q_len + 2)
...
#         max_len = 256  # max_len of BERT input
#         fix_d_len = 205 #self.opt['max_doc']   # fix lenght of document 
#         fix_q_len = max_len - fix_d_len - 3#self.opt['max_len']   # fix length of conversation history (query)
#         batch_size = len(batch['query_tok'])
...
#             q_text = [vocab[int(j.item())] for j in batch['query_tok'][i] if int(j.item())] 
#             # if len(q_text)> fix_q_len:
#             #   print("wow len(q_text): ", len(q_text), "fix_q_len: ", fix_q_len)
#             q_text = " ".join(q_text)            
#             q_tokens = self.tokenizer.tokenize(q_text)
#             q_text = " ".join(q_tokens)
...
#         doc_bert = bert_to_tokens(trunc_doc) 
#         query_bert = bert_to_tokens(trunc_query) 
