...
class myNetwork(nn.Module):
    def __init__(self, decoder, 
        ...
        #self.encoder = encoder  # TODO: DELETED why? no need we just need to modify it to BERT
        self.decoder = decoder
        self.ans_embedding = ans_embedding  #TODO: we do not use it in this module
...
class DocReaderModel(object):
    def __init__(self, opt, embedding, state_dict=None):
        ...
        self.eval_embed_transfer = True  # TODO: Should we delete it ? where we can use it
        self.train_loss = AverageMeter()
        self.len_embeds = opt['embedding_dim'] # i.e. BERT embeds i.e. 768
        self.embedding = embedding


        if self.opt['weight_type'] == 'bleu': # TODO: why we still need it?
        ...
        #TODO: deleted though the BERT should be added here
        ...
        # else:
        #     self.cove_embedder = None # TODO self.cove_embedder  is None for san

        decoder_hidden_size = opt['decoder_hidden_size']

        enc_dec_bridge = nn.Linear(
            self.len_embeds, # TODO: MODIFIED   encoder.hidden_size,
            decoder_hidden_size)
        ...
        decoder = SANDecoder(self.len_embeds,
                             decoder_hidden_size,
                             opt,
                             prefix='decoder',
                             dropout=nn.Dropout(0.2))  # TODO: MODIFIED nn.Dropout()

        ans_embedding = nn.Embedding(opt['vocab_size'],
                                     self.len_embeds,  #TOD from  doc_mem_hidden_size,
                                     padding_idx=0)  #TODO: why do we need answer embeddings?

        print('decoder hidden size: %d' % decoder_hidden_size)
        print('ans emb size: %d' % self.len_embeds)
        ...
        self.network = myNetwork(decoder, ans_embedding, generator,
                                 loss_compute, enc_dec_bridge)
        if state_dict:
        ...
        if opt['optimizer'] == 'sgd':
            ...
            self.optimizer = optim.Adamax(parameters,
                                          opt['learning_rate'],
                                          weight_decay=opt['weight_decay']) # TODO weight_decay
        ...
        if opt['fix_embeddings']:
            ...
            wvec_size = 0 # TODO it is zero anyway so why use it?

        if opt.get('have_lr_scheduler', False):  #  TODO: Why not opposite?????
            ...
            self.scheduler = None
        self.total_param = sum([p.nelement() for p in parameters]) - wvec_size  # TODO why it was calculated this way? why - wvec_size?

    # TODO: DELETED  I think it is not right to delete it

    # ##  RNN encoder + memory
    # def encode_memnet(self, query, batch):  # TODO: what do they mean here by query and batch?
    #     if self.opt['cuda']:
    #         query = query.cuda()
    ...
    ##  RNN encoder    
    # TODO: it is used just in memnet ans seq2seq networks
    def encode(self, query, batch):   
      # TODO: batch['doc_tok'] = query, batch['answer_token'][:, 1:-1]= batch
        #TODO: ADDED
        print('query: ', query)
        print("batch['query_mask'] = ", batch['query_mask'])
        print("batch['query_tok'] = ", batch['query_tok'])
        return #  TODO: ADDED 
        ##################
    ...
    def compute_w(self, fact, res, smooth=0, batch_size=32):  #TODO: 
        def _strip_pad(lst):
    ...
    def update(self, batch, smooth=-1, rep_train=0.5): # TODO: I need to understand inputs
        self.network.train()
        if rep_train > 0:  # TODO: what is this?
            rep_train = 1 - rep_train
            rep_len = int(len(batch['doc_tok'])*rep_train)
            answer_token = batch['answer_token'][:rep_len]  # TODO: what if len(batch['answer_token'])<rep_len
            doc_tok = batch['doc_tok'][rep_len:]
            ans_len = len(batch['answer_token'][1])
            doc_tok = doc_tok[:, :ans_len]  # TODO: whyyyyyy?
        ...
        # compute query_mem
        query_mem = []
        for query_tokens in batch['query_tok']:
            query_tokens_embeds = [self.embedding[int(query_word_id)].tolist() for query_word_id in query_tokens]
            query_embeds = torch.mean(torch.Tensor(query_tokens_embeds), 0)
            query_mem.append(query_embeds.tolist())
        query_mem = torch.Tensor(query_mem)
        # compute doc_mem
        doc_mem = []
        for doc_tokens in batch['doc_tok']:
            doc_tokens_embeds = [self.embedding[int(doc_word_id)].tolist() for doc_word_id in doc_tokens]
            doc_embeds = torch.mean(torch.Tensor(doc_tokens_embeds), 0)
            doc_mem.append(doc_embeds.tolist())
        ...
        print('doc_ans_emb.shape: ', doc_ans_emb.shape) #TODO: ADDED
