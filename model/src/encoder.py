...
class LexiconEncoder(nn.Module):
    ...
    def patch(self, v):
        if self.opt['cuda']:
            v = Variable(v.cuda(non_blocking=True)) # TODO: fixed from async=True to  non_blocking=True
        else:
            v = Variable(v)
        return v
    ...
    def forward(self, batch):
        drnn_input_list = []
        qrnn_input_list = []
        emb = self.embedding  if self.training else self.eval_embed # TODO it was working fine I do not know
