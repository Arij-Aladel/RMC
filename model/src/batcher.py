...
def prepare_batch_data(batch, ground_truth=True):
    ...
    :param ground_truth:
    :return: batch_dict
    batch_dict['doc_tok'] = doc_id ; torch.LongTensor(batch_size, doc_len)
    batch_dict['doc_pos'] = doc_tag ; torch.LongTensor(batch_size, doc_len)
    batch_dict['doc_ner'] = doc_ent ; torch.LongTensor(batch_size, doc_len)
    batch_dict['doc_fea'] = doc_feature ; torch.LongTensor(batch_size, doc_len)
    batch_dict['query_tok'] = query_id ; torch.LongTensor(batch_size, query_len)
    batch_dict['doc_mask'] = doc_mask ; torch.eq(doc_id, 0)
    batch_dict['query_mask'] = query_mask ; torch.eq(query_id, 0)
    and if ground_truth:
       batch_dict['answer_token'] = doc_ans ; torch.LongTensor(batch_size, ans_len + 2)
       batch_dict['ans_mask'] = torch.eq(doc_ans, 0)
    '''
    ...
    for i, sample in enumerate(batch):
        ...
        if ground_truth:
            ...
            answer_tok = [2] + answer_tok_ori + [3] # for answer they add [2] for begining of the answer and [3] for
...
class BatchGen:
    def __init__(self, data_path, batch_size, gpu, is_train=True, doc_maxlen=104):
        self.batch_size = batch_size
        self.doc_maxlen = doc_maxlen
        self.is_train = is_train
        self.gpu = gpu
        self.data_path = data_path
        self.data = self.load(self.data_path, is_train, doc_maxlen)  # list of dictionaries
        if is_train:
            indices = list(range(len(self.data)))
            random.shuffle(indices)
            data = [self.data[i] for i in indices]
        data = [self.data[i:i + batch_size] for i in range(0, len(self.data), batch_size)]  # list of lists
        self.data = data
        self.offset = 0
        ...
        while self.offset < len(self):
            ...
            if self.gpu:
                for k, v in batch_dict.items():
                    batch_dict[k] = v.pin_memory() # TODO I need to understand this
            self.offset += 1
