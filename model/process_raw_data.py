...
#from io import open  # TODO: fixed by me
...
def filter_resp(text):  # remove | symbols
    text = re.sub('\[|\]', '', text)
    return filter_text(text)
    # anything else?


def no_label(fact):  # remove labels
    include_labels = [
...
def load_facts(fact_path):
    ...
    with open(fact_path, 'r', encoding='utf8') as fin:
        for i, line in enumerate(fin):
            ...
            if len(fact) > 100:  # if fact truncated add <TRNC>
                fact = fact[:100] + ['<TRNC>']
...
def combine_fact(fact_dict, anc_type='section', fact_len=12, just_anc=False):
    ...
    for fact_id in fact_dict.keys():
        ...
        if not anc_flag:
            facts = facts[:fact_len]  # 12 fact 12 sentence or section
        else:
...
def load_conv(conv_path, fact_dict, is_train=True, min_que=5):
    ...
    with open(conv_path, 'r', encoding='utf8') as fin:
        for i, line in enumerate(fin):
            ...
            if len(facts) > 500:
                facts = facts[:500] + ['<TRNC>']   # limit facts tokens to 500

            if len(response) > 30:   # limit response tokens to 30
            ...
            conv_fact_list.append(conv_fact_dict)  # list of dictionaries
...
def combine_files(files_path, anc_type='section', fact_len=12, just_anc=False, is_train=False):
    ...
    count_dups = 0
    count_short_facts = 0
    print("from combine_files file_path: ", files_path)
    for file in os.listdir(files_path):
        file_name.add(file[:file.find('.')]) # TODO what is  file.find('.')
    ...
    for file in file_name:
      ...
      data_list += conv_fact  # list of dictionaries
    ...
    return data_list  # list of dictionaries
