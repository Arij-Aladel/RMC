...
class DocReaderModel(object):
    ...
    def update(self, batch, smooth=-1, rep_train=0.5):
        ...
        for query_tokens in batch['query_tok']:
            #TODO: ADDED FILTER TO 128
            query_tokens_embeds = [self.embedding[int(query_word_id)].tolist()[:128] for query_word_id in query_tokens]
            query_embeds = torch.mean(torch.Tensor(query_tokens_embeds), 0)
            query_mem.append(query_embeds.tolist())
        query_mem = torch.Tensor(query_mem).cuda()
        # compute doc_mem
        doc_mem = []
        for doc_tokens in batch['doc_tok']:
            #TODO: ADDED FILTER TO 128
            doc_tokens_embeds = [self.embedding[int(doc_word_id)].tolist()[:128] for doc_word_id in doc_tokens] # (501, ) (doc_len, bert_embeds_len)
            doc_mem.append(doc_tokens_embeds)
        doc_mem = torch.Tensor(doc_mem).cuda()
        # doc_mask
        doc_mask = batch['doc_mask'].cuda()
 
        batch_size = query_mem.size(0)

        hidden = self.network.enc_dec_bridge(query_mem)
        #TO CHECK: same for doc_mem
        ...
        for word in torch.split(doc_ans_emb, 1)[:-1]: # each of size (1, 32, 768)  (1, batch_size, emb_size) 
            word = word.squeeze(0)
            '''
            print('word.shape: ', word.shape) # (32, 768)   ==> [32, 128])
            print('hidden.shape: ', hidden.shape) # (32, 512) ===> (32, 512)
            print('doc_mem.shape: ', doc_mem.shape)  # (32, 768) ===> (32, 501, 128)
            print('doc_mask.shape: ', doc_mem.shape)  #  ===> (8, 501)
            '''
    ...
    def predict(self, batch, top_k=2):
        ...
        for doc_tokens in batch['doc_tok']:
            #TODO: ADDED FILTER TO 128
            doc_tokens_embeds = [self.embedding[int(doc_word_id)].tolist()[:128] for doc_word_id in doc_tokens] # (501, 768) (doc_len, bert_embeds_len)
            doc_mem.append(doc_tokens_embeds)
        doc_mem = torch.Tensor(doc_mem).cuda()
        # doc_mask
        doc_mask = batch['doc_mask'].cuda()
        ...
        # next_token: torch.Tensor([BOS, BOS, ...., BOS]) of shape (32,)
