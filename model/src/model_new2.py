...
class DocReaderModel(object):
    ...
    def update(self, batch, smooth=-1, rep_train=0.5):
        ...
        for doc_tokens in batch['doc_tok']:
            doc_tokens_embeds = [self.embedding[int(doc_word_id)].tolist() for doc_word_id in doc_tokens] # (501, ) (doc_len, bert_embeds_len)
    ...
    def predict(self, batch, top_k=2):
        ...
        doc_mem = []
        for doc_tokens in batch['doc_tok']:
            doc_tokens_embeds = [self.embedding[int(doc_word_id)].tolist() for doc_word_id in doc_tokens] # (501, 768) (doc_len, bert_embeds_len)
            doc_mem.append(doc_tokens_embeds)
        doc_mem = torch.Tensor(doc_mem).cuda()
        # doc_mask
        doc_mask = batch['doc_mask'].cuda()
    ...
    def eval_test_loss(self, batch):
        ...
        with torch.no_grad():
            ...
            for word in torch.split(doc_ans_emb, 1)[:-1]:
                ...
                hidden = self.network.decoder(word, hidden, doc_mem, doc_mask)
                hiddens.append(hidden)
            hiddens = torch.stack(hiddens).cuda()
            log_probs = self.network.generator(hiddens.view(-1, hiddens.size(2)))
