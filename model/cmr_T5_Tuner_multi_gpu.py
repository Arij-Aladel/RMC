#
# created by "Arij Al Adel" (Arij.Adel@gmail.com) at 1/22/21
#
...
class T5FineTuner(pl.LightningModule):
    ...
    def validation_step(self, batch, batch_idx):
        ...
        self.log("self.general_dev_loss", self.general_dev_loss.avg, on_step=True, on_epoch=True,
    ...
    def validation_epoch_end(self, val_step_outputs):
        # if self.is_logger():
            ...
            #     answers.append(val_step_outputs[i]['answers'])
            #     docs.append(val_step_outputs[i]['docs'])
            #     preds.append(val_step_outputs[i]['preds'])
            # answers = torch.cat(answers)
            # docs = torch.cat(docs)
            # preds = torch.cat(preds)

            answers_from_all_process = [torch.zeros_like(answers) for _ in range(dist.get_world_size())]
            dist.all_gather(answers_from_all_process , answers)

            docs_from_all_process = [torch.zeros_like(docs) for _ in range(dist.get_world_size())]
            dist.all_gather(docs_from_all_process, docs)

            preds_from_all_process = [torch.zeros_like(preds) for _ in range(dist.get_world_size())]
            dist.all_gather(preds_from_all_process, preds)
            ...
            self.log("val_loss_acc", val_loss_acc, prog_bar=True, logger=True)
    ...
    def generate_step(self, batch, loss_name):
        ...
        self.log(loss_name, loss, on_step=True, on_epoch=True, prog_bar=True, logger=True)

        answer = [self.tokenizer(x, return_tensors="pt")["input_ids"].flatten() for x in ]
        doc = [self.tokenizer(x, return_tensors="pt")["input_ids"].flatten() for x in batch['doc']]
        print("len answer: ",len(answer))
        print("batch['answer'][0]): ",batch['answer'][0])
        print("decoded answer: ", self.tokenizer.decode(answer[0],
                                                   skip_special_tokens=True, clean_up_tokenization_spaces=True))
        print("answer: ", answer)

        # return {loss_name: loss, "batch_length": len(batch['answer']), "pred": pred, "answer": batch['answer'],
        #         "doc": batch['doc']}
        return {loss_name: loss, "batch_length": len(batch['answer']), "pred": generated_ids, "answer": answer,
                "doc": doc}
