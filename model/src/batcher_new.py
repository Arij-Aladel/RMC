...
def load_meta(opt, meta_path):
    ...
    return opt, meta['vocab']


##TODO: ADDED
print("Loading DistilBERT model and tokenizer... ")
bert_tokenizer = DistilBertTokenizer.from_pretrained('distilbert-base-uncased')
bert_model = DistilBertModel.from_pretrained('distilbert-base-uncased', return_dict=True)
print("DONE")
'''
...
##ADDED
def get_bert_embeds(vocab):
    """
    Returns tensor of BERT embedding of input vocabulary

    Args:
        vocab (my_utils.tokenizer.Vocabulary): input vocabulary

    Returns:
        embeddings (torch.Tensor): tensor of BERT embeddings of vocabulary
            words. Of length (vocabulary_length, embedding_dim)
    """
    if path.exists('bert_vocab_embeddings.pickle'):
        print("\nLoading vocabulary BERT embeddings from pickle file...")
        with open('bert_vocab_embeddings.pickle', 'rb') as f:
            embeddings = pickle.load(f)
        print("DONE")
    else:
        embeddings = []
        len_embeddings = 768 #BERT length of embeddings
        vocab_words = vocab.get_vocab_list()
        print("\nComputing BERT embeddings of vocabulary...")
        for word in tqdm(vocab_words):
            if word in ['<PAD>', '<UNK>', '<BOS>', '<EOS>']:
                curr_embedding = [0.]*len_embeddings
            else:
                input_ids = torch.tensor(bert_tokenizer.encode(word)).unsqueeze(0)  # Batch size 1
                outputs = bert_model(input_ids)
                #print("input_ids: ", input_ids)
                last_hidden_states = outputs[0][0] # The last hidden-state is the first element of the output tuple
                curr_embedding = torch.mean(last_hidden_states, 0).tolist()
                #print("Final output bert embeddings of word '{}' is {} OF length {} and type {}".format(
                #        word, curr_embedding, len(curr_embedding), type(curr_embedding)))
            embeddings.append(curr_embedding)
        print('DONE')
        embeddings = torch.Tensor(embeddings)
        print("Embeddings are of shape: {}", embeddings.shape)
        print("Storing vocabulary BERT embeddings to pickle file...")
        with open('bert_vocab_embeddings.pickle', 'wb') as f:
...
class BatchGen:
    def __init__(self, data_path, batch_size, gpu, is_train=True, doc_maxlen=100):
        ...
        '''
        ##ADDED
        print("first record of first training data batch ({} records): ")
