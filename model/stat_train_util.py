...
def eval_test_loss(model, dev_batches, vocab=None):  # TODO: the issue of GPU is here
    dev_batches.reset()

    tot_loss = 0
    num_data = 0
    for i, (batch,_) in enumerate(dev_batches):
        # print("i / dev_batches: ", i," / ",len(dev_batches))# TODO ADDED
        # TODO ADDED
        # print("GPU Memory usage: %s" % getMemoryUsage())
        loss = model.eval_test_loss(batch, vocab)
        torch.cuda.empty_cache()
        # print("GPU Memory usage after: %s" % getMemoryUsage())
...
def check(model, dev_batches, vocab=None, full_path='', # full path od validation data dev ot test
    ...
    dev_toks = []
    dev_fact = []
    # print("Vocabulary from chech: " , type(vocab))
    for batch, _ in dev_batches:
        prediction, prediction_topks = model.predict(batch, vocab)
    ...
    with open(full_path, 'r', encoding='utf8') as fr:
        ...
        print("pred_words_list: ", len(pred_words_list))
